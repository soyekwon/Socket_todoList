...
while True:
    ...
    print("| 1. todo list 불러오기(GET) ", "2. 추가(POST) ", "3. 수정(PUT) ",
          "4. 삭제(DELETE)", "5. header 요청(HEAD)", "6. 종료")
    ...
    request_data = request_data.format(
        len(todo), todo)  # request message fomatting
    ...
    if num == 1:
        date = response_data_header.split("\r\n")[1].split(
            ":")[1].split(" ")  # header에서 date 추출
        month = date[2].strip()  # date에서 월 추출
        day = date[4].strip()  # date에서 일 추출
        day_of_week = date[1].strip()  # date에서 요일 추출
        response_data_body = data.decode().split("\r\n\r\n")[1].split(
            "\r\n")  # response message에서 body부분 추출
        status_code = response_data_body[0].split(
            ':')[1].lstrip()  # body에서 status code 추출
        todo_list = response_data_body[1].split(
            ':')[1].lstrip().split("\n")   # body에서 todolist 추출
